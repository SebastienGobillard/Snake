// onload permet de lancer l'appli dès l'ouverture de la page
onload = function () {
  // propriete
  ...
  var ctx;
  var delay = 100; // un délai de 1 seconde, en javascript le temps se traduit en milliseconde
  ...
  // methode
  function init() {
    var canvas = document.createElement('canvas'); // je donne une taille
    canvas.width = 900;
    canvas.height = canvasHeight;
    canvas.style.border = '30px solid green'; // je mets une bordure
    canvas.style.margin = "50px auto" // je le centre en haut et en bas de 50px (obligatoirement de type display block)
  ...
  function refreshCanvas() {
    ...
    else 
    {
      if (snakee.isEatingApple(applee))  // si mon serpent à manger la pomme
      {
        ...
        do 
        {
          applee.setNewPosition(); //alors je donne une nouvelle position à ma pommeapplee.setNewPosition(); //alors je donne une nouvelle position à ma pomme
        }
         while(applee.isOnSnake(snakee)) // si isOnsnake, alors renvoi directement sur le do
      }
...
// je choisi dans quel couleur(fillstyles) je veux le dessiner
  ...
  function gameOver() 
  {
    ...
    ctx.textBaseline = "middle";// mon texte se place au milieu et s'aligne au milieu de cet axe
  ...
  function drawScore ()  // j'affiche le score
  {
    ctx.save();
    ...
    var centreX = canvasWidth / 2; // centre sur la ligne horizontal de mon canvas
    var centreY = canvasHeight / 2;
    ctx.textBaseline = "middle";// mon texte se met au milieu et s'aligne au milieu de cet axe
